...
int main(void) {
    ...
    argv = {"binary_name", "--spec-draft-low-yield-fallback", "2"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_SPECULATIVE));
    assert(params.speculative.draft.low_yield_fallback == 2);

    argv = {"binary_name", "--spec-draft-defer-accept-process"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, LLAMA_EXAMPLE_SPECULATIVE));
    assert(params.speculative.draft.defer_accept_process);
